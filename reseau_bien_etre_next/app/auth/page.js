  ...
  function EspacePrive() {
    ...
    // define state to track failed login attempts
    ...
    const doLogin = (e) => {
      ...
      if (failedLoginAttempts >= 4) {
        alert('Maximum login attempts reached. Please contact the administrator.');
        return; // defines message when more than 4 tentatives
      }  
      ...
      resPostLoginCheck.then(
        (res) => {
          if(res?.status === 'error') {
            setFailedLoginAttempts(failedLoginAttempts + 1); // Increment failed attempts
